...
def main():
    ...
    json_file_path = '/UPDS/STT/large_dataset_reve_cv_oslr/merged_dict_reve_cv_oslr.json'
    # json_file_path_own = "/home/asif/stt_all/distil_whisper/distil-whisper/training/json_data_own.json"
    json_file_path_own_small = '/home/asif/stt_all/distil_whisper/distil-whisper/training/json_data_own_small.json'
    # Define the path to prepend to the audio field
    audio_path_prefix = '/UPDS/STT/large_dataset_reve_cv_oslr/'

    # Read the JSON file
    with open(json_file_path_own_small, 'r', encoding='utf-8') as file:
        data = json.load(file)

    ################################################
    # # Iterate through each entry and modify the audio field
    # for key, value in data.items():
    #     original_audio_path = value['audio']
    #     new_audio_path = os.path.join(audio_path_prefix, original_audio_path)
    #     data[key]['audio'] = new_audio_path


    # with open('/home/asif/stt_all/distil_whisper/distil-whisper/training/json_data_own.json', 'w', encoding='utf-8') as file:
    ...
    # accelerate launch run_pseudo_labelling.py --model_name_or_path "openai/whisper-large-v2" --dataset_name "mozilla-foundation/common_voice_13_0" --dataset_config_name "bn" --dataset_split_name "train+validation+test" --text_column_name "sentence" --id_column_name "path" --output_dir "./common_voice_13_0_bn_pseudo_labelled" --wandb_project "distil-whisper-labelling" --per_device_eval_batch_size 64 --dtype "bfloat16" --dataloader_num_workers 16 --preprocessing_num_workers 16 --logging_steps 500 --max_label_length 128 --report_to None --language "bn" --task "transcribe" --return_timestamps --attn_type "flash_attn" --streaming False --generation_num_beams 1 --decode_token_ids False --push_to_hub

    # accelerate launch run_pseudo_labelling.py --model_name_or_path "openai/whisper-large-v2" --dataset_config_name "bn" --dataset_split_name "train+validation+test" --text_column_name "sentence" --id_column_name "path" --output_dir "./common_voice_13_0_bn_pseudo_labelled" --per_device_eval_batch_size 64 --dtype "bfloat16" --dataloader_num_workers 16 --preprocessing_num_workers 16 --logging_steps 500 --max_label_length 128  --return_timestamps --attn_type "flash_attn" --streaming False --generation_num_beams 1 --decode_token_ids False --push_to_hub

    # Now, data contains the modified entries. You can use it directly or write it back to a file.
    # For example, to write it back to a file:
    # with open('/path/to/modified_json.json', 'w', encoding='utf-8') as file:
    #     json.dump(data, file, indent=4)


    ###########################################################

    # Replace this part with your custom dataset loading logic
    # with open('/home/asif/stt_all/distil_whisper/distil-whisper/training/json_data_own.json', 'r', encoding='utf-8') as file:
    #     json_data = json.load(file)

    train_df, val_df, _ = Train_Val_df.generate_df_from_json(data)
    ...
    def prepare_dataset(batch):
        ...
        if id_column_name not in batch:
            # Handle the missing 'path' case here
            # For instance, you can skip this batch or set a default value for 'file_id'
            return batch
